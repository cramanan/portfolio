...
export default function Index() {
    ...
    return (
        <>
            ...
            <header className="sticky z-50 top-0 p-2 rounded-2xl">
                <div className="flex items-center justify-between">
                    <span />
                    <nav
                        className={`flex gap-2 font-medium ${asapItalic.className}`}
                    >
                        <a href="#">{t("home")}</a>
                    ...
                    <div className="flex gap-2">
                        ...
                        >
                            <option className="bg-background" value="fr">
                                🇫🇷
                            </option>
                            <option className="bg-background" value="en">
                                🇺🇸
                            </option>
                        </select>
                    </div>
            ...
            <section className="flex pt-3 flex-col items-center">
                <Image
                    ...
                    width={220}
                    height={220}
                    className="rounded-full border-2 border-foreground border-solid"
                    priority
                />
                <div className="flex flex-col items-center">
                    <h2 className="text-2xl font-medium">Cyril Ram.</h2>
                    <h1
                        className={`text-3xl font-medium ${asapItalic.className}`}
                    >
                        {t("dev1")} {t("dev2")}
                    </h1>
                    <p className="p-5">
            ...
            <section id="skills" className="h-screen">
                <h2 className={asapItalic.className}>{t("skills.title")}</h2>
                <div>
            ...
            <section id="projects" className="h-screen">
                <h2 className={asapItalic.className}>{t("Projects")}</h2>
            </section>
            <section id="contact" className="h-screen">
                <h2 className={asapItalic.className}>{t("contact.title")}</h2>
                <p>{t("contact.description")}</p>
                <div>
                    {socials.map(({ href, title, d }) => (
                        <a key={title} href={href} title={href} target="_blank">
                            <svg
                ...
                <div title="My email: cramananjaonapro@gmail.com">
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
                        width={40}
